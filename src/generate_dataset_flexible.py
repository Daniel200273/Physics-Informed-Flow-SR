...
def run_simulation(sim_id, n_sims, output_dir, resolution, scenario_seed):
    ...
    This ensures that if we run 64x64 and 256x256 with the same seed,
    ...
    # --- SCENARIO SETUP (Resolution Independent) ---
    if scenario == 'plume':
    ...
    for frame in range(FRAMES):
        ...
        while current_time < DT_FRAME:
            ...
            # CFL condition
            # Adjust dt based on resolution cell size
        ...
        # Save Frame
        v_np = v.at_centers().values.numpy('y,x,vector') # (H, W, 2)
        frames_data.append(v_np)
...
def generate_dataset(n_sims, output_dir, lr_res, generate_downscaled):
    ...
    for i in range(n_sims):
        # Unique seed for this scenario pair
        # We use i as seed so HR and LR get same parameters
        ...
        if generate_downscaled:
            # OPTION A: Downscaled (Avg Pooling)
            # Perfect alignment, "Honey" effect removed manually
            print(f"   > Downscaling HR to {lr_res}x{lr_res}...")
            # Reshape for pooling: (Time, H, W, C) -> (Time, h, f, w, f, C)
            f = HR_RES // lr_res
            t, h, w, c = hr_data.shape
            lr_data = hr_data.reshape(t, h//f, f, w//f, f, c).mean(axis=(2, 4))
            mode = "downscaled"
        else:
            # OPTION B: Native Low Res (Independent)
            # Sim-to-Real logic, includes "Honey" viscosity & Divergence
...
if __name__ == "__main__":
    ...
    # Flag: If present, use Downscaling. If absent, use Native Sim.
    parser.add_argument("--downscale", action="store_true", help="If set, LR is generated by pooling HR. If not, LR is a separate native sim.")
